...
fn printSequence(my_seq: anytype) void {
    ...
    switch (my_typeinfo) {
        .Array => {
            ...
            // In the case of the normal array it just prints all of the characters
            // We arent checking for the sentinel here so we will just print out the entire array
            for (my_seq) |s| {
                print("{}", .{s});
            }
        },
        .Pointer => {
            ...
            // This looks like we can create a sentinel based on the type of our input
            // The type of myseq is a u32 array, so we create a u32 sentinel using zig's
            // type introspection and sentinel() function
